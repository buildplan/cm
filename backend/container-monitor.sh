...
_SCRIPT_DEFAULT_LOG_FILE="$DATA_DIR/container-monitor.log"
...
${fail_details}"
        ...
        local issues_json="{"
        local first=true
        for name in "${!CONTAINER_ISSUES_MAP[@]}"; do
            [[ "$first" == "true" ]] || issues_json+=","
            issues_json+=$(jq -n --arg k "$name" --arg v "${CONTAINER_ISSUES_MAP[$name]}" '{ ($k): $v }' | sed 's/[{}]//g')
            first=false
        done
        issues_json+="}"
        new_state_json=$(jq --argjson issues "$issues_json" '.container_issues = $issues' <<< "$new_state_json")
